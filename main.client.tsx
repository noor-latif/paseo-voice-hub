...
export function MainSurface({ theme }: PluginSurfaceProps) {
  ...
  return (
    <ScrollView
      style={{ flex: 1, backgroundColor: colors.surface0, padding: 16, gap: 12 } as never}
    >
        ...
        {(['sv', 'en'] as const).map((v) => (
          <Pressable
            ...
            style={{
              ...
              borderRadius: 20,
              borderWidth: 1,
              borderColor: language === v ? colors.accent : colors.border,
              backgroundColor: language === v ? colors.accent : colors.surface0,
            }}
          >
            <Text
              style={{
                color: language === v ? colors.accentForeground : colors.foreground,
                fontSize: 13,
              }}
            >
              {v}
      ...
      >
        <Text style={{ color: colors.accentForeground, fontWeight: '600' }}>Save and enable</Text>
      </Pressable>
      ...
      <Pressable
        onPress={async () => {
          try {
            ...
            setMsg({ text: r.ok ? `Voice OK ${r.bytes}b` : r.error || 'Voice failed', ok: !!r.ok });
          } catch (e) {
            setMsg({ text: e instanceof Error ? e.message : String(e), ok: false });
          }
        }}
      ...
      >
        <Text style={{ color: colors.foreground }}>Test voice</Text>
      </Pressable>

      {msg && (
        <View
          style={{ padding: 10, borderRadius: 8, backgroundColor: msg.ok ? '#0f5132' : '#842029' }}
        >
