...
async function readSettings(): Promise<VoiceHubSettings> {
  try {
    const raw = await readFile(join(process.env.PASEO_HOME ?? join(homedir(), ".paseo"), "plugins", PLUGIN_ID, "settings.json"), "utf8");
...
async function writeSettings(settings: VoiceHubSettings): Promise<VoiceHubSettings> {
  const path = join(process.env.PASEO_HOME ?? join(homedir(), ".paseo"), "plugins", PLUGIN_ID, "settings.json");
...
async function ensureDaemonSpeechConfig(settings: VoiceHubSettings): Promise<{ changed: boolean; needsRestart: boolean }> {
  ...
  if (next === original + "\n" || JSON.stringify(JSON.parse(next)) === JSON.stringify(JSON.parse(original))) return { changed: false, needsRestart: false };
...
export default function contribute(plugin: PluginContext) {
  ...
  plugin.handle(saveSettings, async ({ groqApiKey, language, sttModel, ttsModel, ttsVoice }) => {
    ...
    const next: VoiceHubSettings = { groqApiKey: trimmed || undefined, language, sttModel, ttsModel, ttsVoice };
  ...
  plugin.handle(testVoice, async ({ text, voice }) => {
    ...
    const v = allowed[(voice || s.ttsVoice || "troy").toLowerCase()] ? (voice || s.ttsVoice || "troy").toLowerCase() : "troy";
    ...
    server = createServer(async (req, res) => {
      ...
      req.on("end", async () => {
        try {
          ...
          const allowed: Record<string, true> = { autumn: true, diana: true, hannah: true, austin: true, daniel: true, troy: true };
          const voice = allowed[(parsed.voice || curSettings.ttsVoice || "troy").toLowerCase()] ? (parsed.voice || curSettings.ttsVoice || "troy").toLowerCase() : "troy";
